"""Playoffs bracket tab for CFB Stat Card Maker."""
...
class PlayoffsTab(ttk.Frame):
    ...
    def _build_season(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(row, text="Season:").pack(side="left")
        self._season_var = tk.StringVar()
        ttk.Spinbox(
            row, from_=2014, to=datetime.datetime.now().year,
            textvariable=self._season_var, width=7,
        ).pack(side="left", padx=(4, 8))
        ttk.Label(row, text="(0=current)", foreground="#555555").pack(side="left")

        ttk.Label(
            lf,
            text="Note: 12-team format began with the 2024 season.",
            foreground="#555555", wraplength=260,
    ...
    def _build_display_options(self, parent: ttk.Frame) -> None:
        ...
        self._show_scores_var  = tk.BooleanVar()
        self._show_seeds_var   = tk.BooleanVar()
        ...
        ttk.Checkbutton(lf, text="Show seeds",
                        variable=self._show_seeds_var).pack(anchor="w", padx=8, pady=1)
        ttk.Checkbutton(lf, text="Show 'data as of' timestamp",
                        variable=self._show_timestamp_var).pack(anchor="w", padx=8, pady=(1, 4))
    ...
    def _fetch_thread(self, bypass_cache: bool) -> None:
        try:
            from app.data.playoffs_api import fetch_playoffs, fetch_playoffs_cached
            from app.cards.playoffs_card import PlayoffsCardConfig, PlayoffsCardRenderer
            ...
            if bypass_cache:
                bracket = fetch_playoffs(season, key)
            else:
                bracket = fetch_playoffs_cached(season, key, ttl)

            config   = self._build_card_config()
            renderer = PlayoffsCardRenderer()
            image    = renderer.render(bracket, config, self.settings.working_dir)
            self.after(0, lambda: self._on_fetch_done(image, None))
        except Exception as exc:
            logger.exception("Playoffs fetch/render failed")
            msg = str(exc)
    ...
    def _build_export(self, parent: ttk.Frame) -> None:
        ...
        self._export_name_var = tk.StringVar(value="playoffs_card")
    ...
    def _export(self, fmt: str) -> None:
        ...
        name = self._export_name_var.get().strip() or "playoffs_card"
    ...
    def _build_card_config(self):
        from app.cards.playoffs_card import PlayoffsCardConfig
        try:
            w = float(self._width_var.get())
        except ValueError:
            w = 10.0
        ...
        return PlayoffsCardConfig(
            ...
            show_seeds=self._show_seeds_var.get(),
    ...
    def _load_from_settings(self) -> None:
        s = self.settings
        self._width_var.set(str(s.playoffs_width_in))
        self._height_var.set(str(s.playoffs_height_in))
        self._use_global_var.set(s.playoffs_use_global_size)
        self._season_var.set(str(s.playoffs_season))
        self._show_logos_var.set(s.playoffs_show_logos)
        self._show_scores_var.set(s.playoffs_show_scores)
        self._show_seeds_var.set(s.playoffs_show_seeds)
        self._show_timestamp_var.set(s.playoffs_show_timestamp)
        self._bg_color_var.set(s.playoffs_bg_color)
        self._export_name_var.set(s.playoffs_export_filename)
        self._append_ts_var.set(s.playoffs_append_timestamp)
    ...
    def apply(self) -> None:
        s = self.settings
        try:
            s.playoffs_width_in = float(self._width_var.get())
        except ValueError:
            pass
        try:
            s.playoffs_height_in = float(self._height_var.get())
        except ValueError:
            pass
        s.playoffs_use_global_size  = self._use_global_var.get()
        s.playoffs_season           = self._get_int(self._season_var, 0)
        s.playoffs_show_logos       = self._show_logos_var.get()
        s.playoffs_show_scores      = self._show_scores_var.get()
        s.playoffs_show_seeds       = self._show_seeds_var.get()
        s.playoffs_show_timestamp   = self._show_timestamp_var.get()
        s.playoffs_bg_color         = self._bg_color_var.get()
        s.playoffs_export_filename  = self._export_name_var.get().strip()
        s.playoffs_append_timestamp = self._append_ts_var.get()
